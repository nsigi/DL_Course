...
def affine_backward(dout, cache):
    ...
    # Вычисляем градиенты по входу, весам и смещениям
...
def batchnorm_forward(x, gamma, beta, bn_param):
    ...
    if mode == "train":
        ...
        x_normalize = (x - _mean) / np.sqrt(_var + eps)
        ...
        cache = (x, x_normalize, _mean, _var, gamma, beta, eps)
...
def batchnorm_backward(dout, cache):
    ...
    x, x_normalize, _mean, _var, gamma, beta, eps = cache
    N, D = dout.shape
    ...
    d_var = np.sum(dx_normalize * (x - _mean) * (-0.5) * ((_var + eps) ** -1.5), axis=0)
    d_mean = np.sum(dx_normalize * (-1 / np.sqrt(_var + eps)), axis=0) \
                   + d_var * (-2 / N) * np.sum(x - _mean, axis=0)
    dx = dx_normalize * (1 / np.sqrt(_var + eps)) + d_var * (2 / N) * (x - _mean) \
         + d_mean * (1 / N)
